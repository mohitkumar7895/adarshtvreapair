    ...
    <div className="min-h-screen bg-admin-bg text-ink">
      <aside className="fixed inset-y-0 left-0 hidden w-64 overflow-y-auto bg-navy text-white lg:block">
        <div className="px-5 py-6">
          <p className="text-xs uppercase tracking-[0.2em] text-copper">Abhishek LED TV Repair</p>
          <p className="mt-1 font-display text-xl">Website admin</p>
          <p className="mt-2 text-xs leading-5 text-white/55">Choose a section, edit the fields, save, then check the live site.</p>
        </div>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex items-center justify-between border-b border-line bg-white px-4 py-3 lg:px-8">
          <p className="text-sm text-muted">
            <span className="font-semibold text-ink">Admin panel</span><span className="mx-2 text-line">/</span> Signed in as <span className="text-ink">{user.name}</span>
          </p>
